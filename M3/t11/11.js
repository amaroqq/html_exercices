...
function t11(){
  for (let i = 0; i < picArray.length; i++){
      ...
      article.setAttribute('onclick', `select(${i})`);
      article.innerHTML = `<h2>${picArray[i].title}</h2><figure><img src="${picArray[i].image.medium}" alt="${picArray[i].title}"><figcaption>${picArray[i].caption}</figcaption></figure><p>${picArray[i].description}</p>`;
